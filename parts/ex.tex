...
\def \glclegend#1#2#3{
  ...
  \node[llegend] at (\lvalignl,\lhfirst) {Processus requis};
  \node[llegend] at (\lvalignl,\lhfirst-\lhsep) {Objectif};
  \node[llegend] at (\lvalignl,\lhfirst-2*\lhsep) {Solution d'un objectif};
  ...
  \ifthenelse{\equal{#1}{prio}}{%
    ...
    \node[llegend] at (\lvalignl,\lhfirst-3*\lhsep) {\phantom{Mg}\\Solution d'un processus\\de sorte coopérative};
