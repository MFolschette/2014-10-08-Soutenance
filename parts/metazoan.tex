...
\begin{frame}[c]
  \frametitle{Frappes de Processus standards}
...
\begin{frame}[t]
  \frametitle{Permissivité de la dynamique standard}
  \framesubtitle{Modèle tiré de \tcite{François \textit{et al.} in Molecular Systems Biology, 2007}}
...
\begin{tikzpicture}
  ...
  \node<2>[darkgreen, align=flush left, text width=3cm] at (7.7,2) {\Large \tval{Production de pigment}};
  \draw<2> (8,4) node[anchor=west, darkgreen] {0 = inactif}
           (8,5) node[anchor=west, darkgreen] {1 = production};
  \draw<3-> (7.75,4.02) node[anchor=west, darkgreen] {= inactif}
  ...
  \node<3>[darkred, align=flush left, text width=2cm, anchor=west] at (1,5.8) {\Large \tval{Horloge}};
  
  \draw<4>[very thick, darkviolet, fill=violet!10] (1,0.5) ellipse [x radius=1cm,y radius=1.5cm];
  \node<4>[darkviolet, align=flush left, anchor=west] at (2,-.3) {\Large \tval{Progression du front d'onde}};
